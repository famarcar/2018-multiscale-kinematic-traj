...
/// UT
    ...
    for(int i=0; i<alphaScales.size(); i++)
        cout<< "value scale: "<< alphaScales[i]<< endl;

    /// fM: 5 April. change files for real testing!!!!!
...
//    computeCumulHist( Codebooks, "train_Exp2_", str_path_trajectories, vec_val_train,
//                      featLabels, vec_activities, alphaScales,  numScaleAllowed, cumulated, num_scalesTrajectories );
//
//    computeCumulHist( Codebooks, "test_Exp2_", str_path_trajectories, vec_test,
//                      featLabels, vec_activities , alphaScales,  numScaleAllowed, cumulated, num_scalesTrajectories);
    ...
    computeFrameLevelHist( Codebooks, "train_Exp1_", str_path_trajectories, vec_val_train,
                           featLabels, vec_activities, alphaScales,  numScaleAllowed, cumulated, num_scalesTrajectories );
//
//
    computeFrameLevelHist( Codebooks, "test_Exp1_", str_path_trajectories, vec_test,
                           featLabels, vec_activities , alphaScales,  numScaleAllowed, cumulated, num_scalesTrajectories);
...
void ToLoadFiles_UT()
{
/// * Note ** in not a test and train, preserve the names to follow KTH scheme
    cout<< str_path_test<< endl;
    redFileSequences(str_path_test, vec_test); ///UT_seq1
    redFileSequences(str_path_train, vec_val_train); ///UT_seq2
    redFileSequences(str_path_activities, vec_activities);
